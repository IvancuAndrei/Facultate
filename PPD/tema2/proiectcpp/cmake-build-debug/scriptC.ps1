...
if (!(Test-Path outC.csv)){
    ...
    #Scrie date in csv
    Set-Content outC.csv 'Tip Paralelism, Test, Nr Threaduri,Timp executie'
}
...
for ($k = 0; $k -lt $tests.Length; $k++) {
    ...
    for ($i = 0; $i -lt $param2; $i++) {
        ...
        $a = (cmd /c .\$param1 $input $tipuri[0] 1 2`>`&1)
        $suma += $a
    }
    $media = $suma / $param2
    Write-Host "Timp de executie mediu:" $media
    Add-Content outC.csv "$($tipuri[0]),$($tests[$k][0]) $($tests[$k][1]) $($tests[$k][2]),1,$($media)"
}
    ...
    for ($k = 0; $k -lt $tests.Length; $k++) {
        for ($j = 0; $j -lt $threaduri[$k].Length; $j++) {
            ...
            for ($i = 0; $i -lt $param2; $i++) {
                ...
                $a = (cmd /c .\$param1 $input $tipuri[$z] $threaduri[$k][$j] 2`>`&1)
                $suma += $a
            }
            $media = $suma / $param2
            Write-Host "Timp de executie mediu:" $media
            Add-Content outC.csv "$($tipuri[$z]),$($tests[$k][0]) $($tests[$k][1]) $($tests[$k][2]),$($threaduri[$k][$j]),$($media)"
